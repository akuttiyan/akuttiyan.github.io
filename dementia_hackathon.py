...
def input_function():
    apoe_4_presence = input('''Do you have the presence of the apoe_4 gene?(press the number). If you aren't sure, just press 1.
                   1. Negative
                   2. Positive
    ...
    smoking_status = input('''What is your smoking status? (press number)
                    1. Current Smoker
                    2. Former Smoker
                    3. Never Smoked
                           ''')
    
    chronic_health_conditions = input(''' Do you have any chronic health conditions?(press number)
                    1. Diabetes
                    2. Heart Disease
                    3. Hypertension
                    4. None
    ...
    education_level = input(''' What is your highest level of education?(press number)
                    1. Primary School
                    2. Secondary School
                    3. No School
                    4. Diploma/Degree
    ...
    cognitive_test_score = input('''What is your cognitive test score?(Ranges from 1 to 10)
                            ''')

    info_list = [ float(cognitive_test_score),  float(smoking_status) - 1, float(apoe_4_presence) - 1, float(chronic_health_conditions) - 1, float(education_level) - 1,]

    return info_list


dementia_data = input_function()
...
# The argmax function finds which of these 2 numbers is larger and print its index
# The index is either 0 or 1 and this tells us what the prediction is
test_predictions_1_0 = np.argmax(test_predictions, axis=1)

num_correct = 0
total = 0

# Loops through the decoded predictions and te expected vales
for i in range(len(test_predictions_1_0)):
    # If the prediction is correct
    if int(expected[i]) == test_predictions_1_0[i]:
        # Add 1 to the number of correct predictions
        num_correct += 1
    # Add 1 to the total no matter what
    total += 1
...
#print(accuracy) # Around 0.90

prepared_dementia_data = []

prepared_dementia_data.append(dementia_data)

real_predictions = model.predict(prepared_dementia_data)

real_predictions_1_0 = np.argmax(real_predictions, axis=1)

if real_predictions_1_0 == [1]:
    print("There is a higher likelihood that you will have dementia")
elif real_predictions_1_0 == [0]:
    print("There is a lower likelihood that you will have dementia")
